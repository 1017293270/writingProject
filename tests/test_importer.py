...
from writing_project.models import Chapter
...
def test_import_task_output_refuses_final_chapter(fake_repository, tmp_path):
    output = tmp_path / "ch001.md"
    output.write_text("# 雪夜入城\n风雪压城。", encoding="utf-8")
    fake_repository.task = fake_repository.task.__class__(**{**fake_repository.task.__dict__, "output_path": str(output)})
    fake_repository.chapter = Chapter(
        **{**fake_repository.chapter.__dict__, "status": "final", "final_path": "E:/novels/demo/chapters/ch001-final.md"}
    )

    with pytest.raises(ValueError, match="already has final content"):
        import_task_output(fake_repository, 9)

    assert not hasattr(fake_repository, "chapter_update")
